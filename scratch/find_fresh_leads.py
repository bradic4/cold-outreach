...
candidate_domains = [
    ...
    ("Dental Tourism Serbia", "https://www.dentaltourismserbia.com/"),
    ("Ordinacija Dentalux", "https://dentalux.rs/"),
    ("Dr Popovic Dental", "https://drpopovic.com/"),
    ...
    ("Ordinacija Cvejanovic", "https://ordinacijacvejanovic.com/"),
    ("Dental Art", "https://dentalart.rs/"),
    ...
    ("Viessmann Srbija", "https://www.viessmann.rs/"),
    ...
    ("Opsta Bolnica Avala", "https://bolnicaavala.rs/"),
    ("Diva Clinic", "https://divalasercentar.com/"),
    ("Dr Colic Hospital", "https://dr-colic.com/"),
    ("Belprime Clinic", "https://belprime.rs/"),
...
for name, url in candidate_domains:
    try:
        ...
        with urllib.request.urlopen(req, context=ctx, timeout=4) as resp:
            ...
            for e in raw_emails:
                ...
                if not any(e_clean.endswith(ext) for ext in ('.png', '.jpg', '.jpeg', '.gif', '.svg', '.webp', '.css', '.js', '.avif')):
                    if not any(x in e_clean for x in ['sentry', 'wix', 'example', 'domain', 'schema', 'test', 'bootstrap', 'wordpress', 'cloudflare']):
                        if e_clean not in history:
...
print(f"\nTotal fresh leads found: {len(found_leads)}")
