...
with open("scratch/ready_leads_batch3.json", "r", encoding="utf-8") as f:
    leads = json.load(f)

print(f"Pokrecem slanje za {len(leads)} verifikovanih kontakata iz Batch 3...\n")
sys.stdout.flush()
...
for idx, lead in enumerate(leads, 1):
    ...
    if niche == "eye_clinic":
        intro = f"<p>Poštovani,</p><p>Pratim rad i reputaciju ustanove {name}. Pacijenti koji razmatraju lasersko skidanje dioptrije ili operacije katarakte (procedure od 1.000€–3.000€) odluku donose pažljivim istraživanjem sa telefona — sajt mora u prve 3 sekunde uliti maksimalno poverenje.</p>"
    elif niche == "architecture":
        intro = f"<p>Zdravo,</p><p>Pratim portfolio i projekte {name}. Renderi i fotografije izvedenih radova se na mobilnoj 4G mreži učitavaju oko 5 sekundi, pa deo potencijalnih klijenata i investitora ode sa sajta pre nego što sagleda sve reference.</p>"
    ...
    elif niche == "luxury_wellness":
        intro = f"<p>Zdravo,</p><p>Pratim premijum projekte i ponudu {name}. Kod visokobudžetnih investicija (spa centri, bazeni, saune, enterijeri), sajt na mobilnom telefonu mora u sekundi prikazati ekskluzivnost i omogućiti lako slanje zahteva.</p>"
    else: # dental
        intro = f"<p>Zdravo,</p><p>Pratim rad i stomatološku praksu {name}. Pacijenti iz inostranstva i domaći klijenti koji traže stomatološke ili implantološke usluge odluku donose direktno preko telefona — sajt je prva ordinacija koju vide.</p>"
