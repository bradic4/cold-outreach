...
check_urls = [
    ("Ginekoloska ordinacija Demetra", "https://demetra.rs/"),
    ("Ginekoloska ordinacija Biljana", "https://drzivaljevic.com/"),
    ("Ginekoloska ordinacija Mladenovic", "https://ordinacijamladenovic.rs/"),
    ("Etno Selo Babina Reka", "https://babinareka.rs/"),
    ("Klub Reset Novi Sad", "https://klubreset.com/"),
    ("Air System Ventilacija", "https://airsystem.rs/"),
    ("Aeroteh Inzenjering", "https://aeroteh.rs/"),
    ("Montazne Kuce Negal", "https://negal.rs/"),
    ("Kontejneri Beograd", "https://kontejneri.rs/")
]
                    ...
                    if not any(ec.endswith(ext) for ext in ('.png', '.jpg', '.jpeg', '.gif', '.svg', '.webp', '.css', '.js', '.avif')):
                        if not any(x in ec for x in ['sentry', 'wix', 'example', 'domain', 'schema', 'test', 'bootstrap', 'wordpress', 'cloudflare', 'contact@yourdomain', 'email@', 'posao', 'job', 'privacy']):
                            if ec not in history:
                                print(f"FOUND B10 MORE: {name} -> {ec}")
                                break
        except Exception:
            pass
